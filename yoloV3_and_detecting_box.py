"""
Note: all the work bellow is built on experiencor's YoloV3, and it is unfinished yet
"""
...
def make_yolov3_model():
    ...
    x = _conv_block(x, [{'filter': 256, 'kernel': 1, 'stride': 1, 'bnorm': True, 'leaky': True, 'layer_idx': 84}],
                    skip=False)
    x = UpSampling2D(2)(x)
    x = Concatenate()([x, skip_61])  # TODO: check for usage
    ...
    x = _conv_block(x, [{'filter': 128, 'kernel': 1, 'stride': 1, 'bnorm': True, 'leaky': True, 'layer_idx': 96}],
                    skip=False)
    x = UpSampling2D(2)(x)
    x = Concatenate()([x, skip_36])  # TODO: check for usage
